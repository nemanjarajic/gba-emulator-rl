...
from gba_env._native import emulator_root  # noqa: E402

# The test ROM is generated by the emulator's tools, so it comes from that
# checkout rather than from this one.
EMULATOR = emulator_root()
ROM = os.path.join(EMULATOR, "build", "roms", "input_echo.gba") if EMULATOR else ""
...
def main():
    if not EMULATOR:
        print("cannot find the emulator checkout.\n"
              "Set GBA_EMULATOR_ROOT, or put it beside this repository as 'gba'.")
        return 2
    if not os.path.exists(ROM):
        print(f"missing {ROM}\n"
              f"run, in the emulator checkout: python3 tools/make_bench_roms.py")
        return 2
    ...
    with GbaVecEnv(ROM, num_instances=n) as env:
        ...
        path = os.path.join(os.path.dirname(__file__), "smoke.state")
